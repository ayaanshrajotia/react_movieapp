...
//     return `http://www.themoviedb.org/t/p/original/${posterPath}`;
// };
...
const SeriesDetails = () => {
        ...
        const fetchDetails = async () => {
            try {
                const details = await axios.get(
                    `/tv/${params.seriesId}?api_key=${API_KEY}&language=en-US`
                );

                const cast = await axios.get(
                    `/tv/${details.data.id}/credits?api_key=${API_KEY}&language=en-US`
                );
                setMovieCharacter(cast.data.cast);
                setSeriesDetails(details.data);
                setLoading(false);
            } catch {
                setError(true);
                setLoading(false);
    ...
    const handleClick = (series) => {
        if (trailerUrl) {
            ...
            setShowTrailer(true);
            movieTrailer(series?.original_name || "")
                .then((url) => {
    ...
    return (
        <div className={`movie__details`}>
            {loading ? (
                ...
                <>
                    {seriesDetails.backdrop_path !== null ? (
                        <img
                    ...
                    <div className="flex_container">
                        <div className="movie__desc_container">
                            <div className="movie__desc">
                                <h1 className="movie__desc_title">
                                    {seriesDetails.original_name}
                                </h1>
                                <h2 className="movie__desc_date">
                                    Last aired on {seriesDetails.last_air_date}
                                </h2>
                                <span
                                    className={`movie__vote ${
                                        seriesDetails.vote_average < 4
                                            ? `red-vote`
                                            : seriesDetails.vote_average > 8
                                            ? `green-vote`
                                            : `orange-vote`
                                    }`}
                                >
                            ...
                            <div className="movie__trailer_button">
                                ...
                                <h3 className="movie__trailer_span">
                                    Watch Trailer
                                </h3>
                            </div>
                        </div>
